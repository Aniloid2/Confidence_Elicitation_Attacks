...
class QQPPrompts(BaseClassificationPrompt):
    ...
    def _predict_prompt(self, text):
        prompt = f"""{self.start_prompt_header}You're a language model that is trying to detect whether two questions are duplicate. Provide your {self.k_pred} best guess for the following text ({self.prediction_options}). Give ONLY the guesses, no other words or explanation.\n\nFor example:\n\nGuesses: <most likely guesses, either {self.prediction_options}; not a complete sentence, just the guesses! Separated by a comma, for example [{self.prediction_options} ... x{self.k_pred}]>\n\nThe first question is: {text[0]}\n\nThe second question is: {text[1]}\n\n Guesses:{self.end_prompt_footer}"""
